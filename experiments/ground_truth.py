...
def main():
    ...
    try:
        with open(fn_mutants, 'r') as f:
            mutants = [boggart.Mutation.from_dict(jsn['mutation'])
                       for jsn in json.load(f)]
            logger.debug("loaded %d mutants from database", len(mutants))
    except Exception:
        logger.exception("failed to load mutant database: %s", fn_mutants)
        sys.exit(1)
    except FileNotFoundError:
        logger.error("mutant database file not found: %s", fn_mutants)
        sys.exit(1)
